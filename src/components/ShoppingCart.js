...
const ShoppingCart = () => {
  ...
  // Funkcja do synchronizacji koszyka z backendem lub `localStorage`
  const fetchCartData = async () => {
    if (cart && cart.items && products) {
      ...
      setLocalCart(detailedCart);
      localStorage.setItem('cart', JSON.stringify({ items: detailedCart }));
    } else {
      const savedCart = localStorage.getItem('cart');
      if (savedCart) {
        try {
          const parsedCart = JSON.parse(savedCart);
          setLocalCart(parsedCart.items || []);
        } catch (error) {
  ...
  return (
    <div className="w-full md:max-w-1/2 mt-0 !space-x-0 !ml-0 h-full overflow-scroll custom-scrollbar bg-white px-4 rounded-lg shadow-lg relative py-0">
      ...
      <div className="sticky w-full bg-text1 bottom-0 z-10 border-t p-4 flex justify-between items-center">
        <h3 className="text-lg font-semibold text-green-600">
